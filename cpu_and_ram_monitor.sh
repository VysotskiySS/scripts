...
# ===== НАСТРОЙКИ =====
INTERVAL=120                    # Интервал между проверками в секундах
...
# Минимальное количество измерений, необходимое для заполнения окна WINDOW
MIN_SAMPLES=$(( (WINDOW + INTERVAL - 1) / INTERVAL ))
# =====================
                ...
                if [[ -n "${prev_cpu_times[$pid]}" ]]; then
                    ...
                    if [ $delta_ticks -lt 0 ]; then
                        ...
                        if [ -n "$cpu_percent" ]; then
                            ...
                            # Проверяем, накоплено ли достаточно данных для полного окна
                            line_count=$(wc -l < "$history_file" 2>/dev/null || echo 0)
                            if [ "$line_count" -ge "$MIN_SAMPLES" ]; then
